...
export class RoomRentComponent implements OnInit {
  ...
  getPropertyId(){
    this.propertyTypeId = this.propertyTypes.find((x:any) => x.name === 'pokój').id;
  }

  setDefaultFilters(){
    this.defaultFilters[0].value = this.propertyTypes.find((x:any) => x.name === 'pokój').id;
    this.defaultFilters[1].value = this.offerTypes.find((x:any) => x.name === 'wynajem').id;
  ...
  getParameters(){
    this.apiService.getParametersForPropertyType(this.propertyTypeId)
      .subscribe((value: any) => {
        ...
        this.roomTypeOptions = value.find((x: any) => x.name === 'liczba osób w pokoju').parameter_values;

        this.additionalInfOptions = value.filter((x: any) => (x.parameter_category !== null && x.parameter_category.name === 'informacje dodatkowe'));
        this.equipmentOptions = value.filter((x: any) => (x.parameter_category !== null && x.parameter_category.name === 'wyposażenie'));
      });
  }
  ...
  onSubmit(){
    ...
    if(this.form.controls['areaRoomFrom'].value !== null && this.form.controls['areaRoomFrom'].value !== ''){
      this.form.controls['areaRoomFrom'].setValue(this.form.controls['areaRoomFrom'].value);
      this.filters.push({
    ...
    if(this.form.controls['areaRoomTo'].value !== null && this.form.controls['areaRoomTo'].value !== ''){
      this.form.controls['areaRoomTo'].setValue(this.form.controls['areaRoomTo'].value);
      this.filters.push({
        column: 'area_square_meters',
        operator: '<=',
    ...
    if(this.form.controls['roomType'].value !== null && this.form.controls['roomType'].value.length > 0){
      this.form.controls['roomType'].setValue(this.form.controls['roomType'].value);
      this.parameterValueIn.push({
    ...
    if(this.form.controls['equipment'].value !== null){
      this.form.controls['equipment'].setValue(this.form.controls['equipment'].value);
